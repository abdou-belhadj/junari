...
class CropsProcess(models.Model):
    ...
    status = fields.Selection(
        ...
        selection=[
            ('01_new', 'New'),
            ('02_in_process', 'In Process'),
            ('03_done', 'Done'),
            ('04_cancelled', 'Cancelled'),
        ],
        group_expand=lambda self, stages, domain, order: ['01_new', '02_in_process', '03_done', '04_cancelled'],
        default='new',
        index=True)
    ...
    def get_description(self):
        for rec in self:
            rec.description = rec.process_id.description
